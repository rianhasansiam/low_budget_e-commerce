...
const socialLinks = [
  ...
  },
  {
    ...
    color: 'bg-gradient-to-br from-purple-600 via-pink-500 to-sky-400 hover:from-purple-700 hover:via-pink-600 hover:to-sky-500',
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <div ref={containerRef} className="fixed bottom-6 right-6 z-50">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-3 min-w-[200px]">
              ...
              <div className="space-y-2">
                {socialLinks.map((social, index) => (
                  <motion.a
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-white transition-all ${social.color}`}
                  >
                    {social.icon}
      ...
      <button
        ...
        className={`relative w-14 h-14 rounded-full shadow-lg flex items-center justify-center transition-all duration-300 bg-gray-900 hover:bg-gray-800 text-white`}
      >
        {isOpen ? (
          <X className="w-6 h-6" />
        ) : (
