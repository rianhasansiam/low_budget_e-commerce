...
// Product data structure normalization - Matches exact database schema
export const normalizeProduct = (product) => {
  ...
  return {
    ...
    _id: product._id,
    id: product._id, // Alias for easier access
    name: product.name || '',
    brand: product.brand || '', // Brand/manufacturer name
    ...
    primaryImage: product.image || (Array.isArray(product.images) && product.images[0]) || '',
    
    // Colors array (required field)
    colors: Array.isArray(product.colors) && product.colors.length > 0
      ? product.colors
      : (product.color ? [product.color] : []),
    
    // Primary color (backward compatibility)
    color: product.color || (Array.isArray(product.colors) && product.colors[0]) || '',
    
    // Sizes array (required field - sensor types for cameras)
    ...
    isLowStock: Number(product.stock) > 0 && Number(product.stock) < 10,
...
export const normalizeReview = (review) => {
  ...
  return {
    ...
    userAvatar: review.userAvatar || review.avatar || null, // Include avatar
    customer: {
      name: review.userName || review.customerName || 'Anonymous',
      email: review.userEmail || review.customerEmail || '',
      avatar: review.userAvatar || review.avatar || null
    },
    rating: Number(review.rating) || 0,
    comment: review.comment || '',
    photo: review.photo,
    title: review.title || '',
    status: review.isApproved ? 'approved' : 'pending', // Map isApproved to status
    ...
    hasAvatar: Boolean(review.userAvatar || review.avatar),
